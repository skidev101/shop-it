...
export function ProductCard({ 
  ...
  return (
    <div className={cn("group relative flex flex-col bg-white rounded-3xl transition-all duration-500 hover:shadow-2xl hover:shadow-black/5 p-4 border border-transparent hover:border-[#F5F5F7]", className)}>
      {/* Image Container */}
        ...
        {badge && (
          <div className="absolute top-4 left-4 z-10 flex items-center gap-1.5 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm border border-black/5">
            <div className={cn(
                ...
                badge === 'BEST PRICE' || badge === 'BESTSELLER' ? "bg-orange-500" : "bg-blue-500"
            )} />
            <span className="text-[9px] font-black text-[#1A1A1A] uppercase tracking-widest">{badge}</span>
          </div>
        )}
        ...
        {(vendorName || vendorAvatar) && (
            <div className="flex items-center gap-2.5 mb-6 pt-1">
                ...
                <p className="text-[11px] font-medium text-[#999999]">
                    Sold by <span className="font-bold text-[#1A1A1A]">{vendorName}</span>
                </p>
            </div>
        )}
