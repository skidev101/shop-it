...
export function DirectoryVendorCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#F5F5F7] p-6 hover:shadow-xl hover:border-transparent transition-all duration-300 group flex flex-col h-full">
      ...
      {/* Featured Products */}
      <div className="mb-6">
        <p className="text-[10px] font-black text-[#999999] uppercase tracking-widest mb-3">Featured Products</p>
        <div className="grid grid-cols-3 gap-3">
          {thumbnails.map((thumb, idx) => (
            <div key={idx} className="aspect-square bg-[#F5F5F7] rounded-lg overflow-hidden relative group/thumb">
              <Image 
                src={thumb} 
                alt={`Product ${idx + 1}`} 
                fill 
      ...
      <div className="mt-auto pt-4 border-t border-[#F5F5F7] flex items-center justify-between">
        <div className="flex flex-col">
           <span className="text-[10px] font-medium text-[#999999]">Est. {estDate} • {sales} Sales</span>
        </div>
