...
function AccountStatCard({ 
  ...
  return (
    <Link href={href} className="block group">
      <Card className={cn("border-none bg-[#F5F5F7] transition-all duration-300 group-hover:bg-[#1A1A1A] group-hover:shadow-xl", className)}>
    ...
    <div className="space-y-12">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-bold tracking-tight text-[#1A1A1A]">Recent Orders</h3>
          <Link href="/account/orders" className="flex items-center gap-1 text-[10px] font-black text-[#1A1A1A] hover:opacity-70 transition-opacity uppercase tracking-widest">
        ...
        <div className="grid gap-4">
          {recentOrders.map((order) => (
            <Link key={order.id} href={`/account/orders/${order.id.replace('#', '')}`} className="group">
              <div className="flex flex-col sm:flex-row items-center justify-between p-6 rounded-2xl bg-white border border-[#F5F5F7] group-hover:border-[#1A1A1A] group-hover:shadow-lg transition-all duration-300 gap-6">
                <div className="flex items-center gap-6 w-full sm:w-auto">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-bold text-[#999999] mt-0.5">{order.date}</p>
                  </div>
                </div>

                <div className="flex items-center justify-between w-full sm:w-auto sm:gap-12 lg:gap-24">
                  <div className="text-center sm:text-left">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#999999] mb-1">Status</p>
                    <div className={cn("flex items-center gap-1.5 text-[11px] font-black uppercase tracking-widest", order.statusColor)}>
                  ...
                  <div className="text-center sm:text-left">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#999999] mb-1">Items</p>
                    <p className="text-[11px] font-black text-[#1A1A1A] uppercase">{order.items} {order.items > 1 ? 'Items' : 'Item'}</p>
                  </div>

                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#999999] mb-1">Total</p>
                    <p className="text-[11px] font-black text-[#1A1A1A]">{order.total}</p>
                  </div>
                </div>
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-base font-black text-[#1A1A1A]">{user?.name || 'Member'}</h4>
              ...
              <div className="pt-4 border-t border-[#F5F5F7] space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[11px] font-black text-[#1A1A1A]">January 2024</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[11px] font-bold text-[#999999] uppercase tracking-wider">Default Address</span>
                  <span className="text-[11px] font-black text-[#1A1A1A]">Lagos, Nigeria</span>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-[#F5F5F7]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="text-[11px] font-black text-[#1A1A1A] uppercase tracking-wide">Email Verified</h5>
                    <p className="text-[10px] text-[#999999] font-medium">Your email is protected.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-[#F5F5F7]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="text-[11px] font-black text-[#1A1A1A] uppercase tracking-wide">2FA Disabled</h5>
                    <p className="text-[10px] text-[#999999] font-medium">Add another layer of security.</p>
                  </div>
                </div>
                <Button variant="outline" className="h-8 px-3 rounded-lg text-[9px] font-black uppercase tracking-widest border-[#E5E5E5]">Enable</Button>
              </div>
            </div>
          </CardContent>
        </Card>
